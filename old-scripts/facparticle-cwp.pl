...
sub main 
{
	...
	my ($wfID, $marcRelatorCode, $authorOrder, $family, $given, $given2, $shortname, $dept, $school, $title, $host, $enum1, $enum2, $chron2, $chron1, $chron3, $chron4, $startPage, $endPage, $pageList, $issn, $type, $url ,$doi, $ready, $version, $authors, $digitalOrigin, $accessCondition, $abstract, $titleNew, $class, $fileName, $keywords, $sponsors, $conference);
	...
	my $usedRange = $Sheet->UsedRange()->{Value};
		...
		{
			$fh->print("<mods:mods>\n\n");
			($title, $authors, $chron1, $chron2, $chron3, $chron4, $conference, $abstract, $sponsors, $fileName, $url) = @$row;
			mods_title($fh, $title);
			mods_name_element($fh, $authors, $data);
			mods_conference ($fh, $conference);
			mods_type_of_resource($fh);
			mods_genre($fh);
			mods_origin_info($fh, $chron1, $chron2, $chron3, $chron4);
			mods_language($fh);
...
$fh->print("<mods:genre authority=\"local\" type=\"work type\" usage=\"primary\">report<\/mods:genre>\n\n");
...
my ($fh, $chron1, $chron2, $chron3, $chron4) = @_;

$fh->print("<mods:originInfo>\n");
	$fh->print("\t<mods:place>\n\t\t<mods:placeTerm type=\"text\">Chestnut Hill, Mass.<\/mods:placeTerm>\n\t<\/mods:place>\n");
	$fh->print("\t<mods:publisher>Center on Wealth and Philanthropy at Boston College<\/mods:publisher>\n");
	if ($chron4) {$fh->print("\t<mods:dateIssued>$chron3 $chron4, $chron2<\/mods:dateIssued>\n");}
	elsif ($chron3) {$fh->print("\t<mods:dateIssued>$chron3 $chron2<\/mods:dateIssued>\n");}
	else {$fh->print("\t<mods:dateIssued>$chron2<\/mods:dateIssued>\n");}
	$fh->print("\t<mods:dateIssued encoding=\"w3cdtf\" keyDate=\"yes\">$chron1<\/mods:dateIssued>\n");
...
my ($fh, $fileName) = @_;
	...
	$fh->print("<localCollectionName>cwp<\/localCollectionName>\n\t");
...
$fh->print ("<mods:name type=\"conference\">\n\t<mods:namePart>$conference<\/mods:namePart>\n\t<mods:displayForm>$conference<\/mods:displayForm>\n\t<mods:role>\n\t\t<mods:roleTerm type=\"text\" authority=\"marcrelator\">Host institution<\/mods:roleTerm>\n\t\t<mods:roleTerm type=\"code\" authority=\"marcrelator\">his<\/mods:roleTerm>\n\t<\/mods:role>\n<\/mods:name>\n\n");
}
